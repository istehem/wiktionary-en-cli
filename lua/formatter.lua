...
local function sounds_to_strings(sounds, padding)
  ...
  for i, v in pairs(sounds) do
    if v.ipa then
      local formatted = string.format(" %s. IPA: %s %s", style_italic_dimmed(i, padding), v.ipa, format_tags(v.tags))
      table.insert(result, formatted)
    end
    if v.enpr then
      local formatted = string.format(" %s. enPr: %s %s", style_italic_dimmed(i, padding), v.enpr, format_tags(v.tags))
      table.insert(result, formatted)
    end
    if v.other then
      local formatted =
...
local function format_did_you_mean_banner(did_you_mean)
  local content = {}
  local searched_for_styled = api.apply_style(style_bold_with_color(did_you_mean.searched_for, "red"), "strikethrough")
